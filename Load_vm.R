...
# 28 Nov 2023
#
# Description: this script loads Mike Dinniman's
...
############ Process the density file
dens_data <- read_lines("density.csv")
lines_length <- length(dens_data)
...
for (line in 1:lines_length) {
  if (!time_counter){
    time_step_str <- strsplit(dens_data[[line]], ",")
    if (line == 1) {
      time_step <- as.numeric(time_step_str)
      new_date <- date_start + seconds(as.numeric(time_step_str))
      date_step <- new_date
